...
# Flask-Migrate 3.0+ não usa mais MigrateCommand - usar Flask CLI em vez de Flask-Script
from flask_migrate import Migrate
from flask_sqlalchemy import SQLAlchemy
# Flask-Script está deprecado - usar Flask CLI nativo
...
# SQLALCHEMY_COMMIT_ON_TEARDOWN removido no Flask-SQLAlchemy 3.0+
# Use db.session.commit() explicitamente quando necessário
...
# Inicializa Flask-Migrate (comandos via Flask CLI: flask db init, flask db migrate, flask db upgrade)
migrate = Migrate(app, db)

# Flask-Script Manager deprecado - comandos disponíveis via Flask CLI nativo
# Para migrations: flask db migrate, flask db upgrade, etc.
# manager = Manager(app)
...
# @app.before_first_request deprecado no Flask 2.3+
# Movido para run_server() com app.app_context()
def create_database():
...
def set_admin():
    ...
    user.role = 'Admin'
    db.session.commit()  # Commit necessário (SQLALCHEMY_COMMIT_ON_TEARDOWN removido)
    users = User.query.all()
    return render_template('manage_users.html', users=users)
...
def check_user_roles():
    """Define role Admin para usuários existentes que não têm role"""
...
def run_server(secure, host='0.0.0.0', port=5000):
    with app.app_context():
        # Inicializa database e roles (substituindo @app.before_first_request deprecado)
        create_database()
        check_user_roles()
        
        # Use app.config instead of login_manager._login_disabled (deprecated)
        app.config['LOGIN_DISABLED'] = not secure

    app.run(host=host, port=port, debug=None)


if __name__ == '__main__':
    # Flask-Script manager.run() deprecado - usar Flask CLI ou run_server diretamente
    # Para comandos de migration: flask db migrate, flask db upgrade
    # manager.run()
    
    # Executa servidor com segurança habilitada
    run_server(secure=True)
